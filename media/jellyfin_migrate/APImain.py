# #############################################################################
# NOTE: 2024-01-14 
#   Shamelessly stolen from https://github.com/Marc-Vieg/Emby2Jelly/tree/master 
#   & hacked to make it work for me when going from Jellyfin to Jellyfin...
#
# #############################################################################
...
def createConfig(path):
    ...
    SourceSection = {
        ...
        'SOURCE_URLBASE': 'http://127.0.0.1:8096/emby/'
    }
	...
	def get_watched_status():
		...
		for user in users:
			...
			if (user['Name'] in selectedUsers) and (user['Name'] is not None) :
				...
				api_url = '{0}Users/{1}/Items?Filters=IsPlayed&IncludeItemTypes=Movie,Episode&Recursive=True&api_key={2}'.format(SOURCE_URLBASE,user['Id'],SOURCE_APIKEY)
				SOURCE_HEADERS_Auth = {'accept': 'application/json',
				   'api_key': '{0}'.format(SOURCE_APIKEY)}
				...
				if response.status_code == 200:
					...
					for item in UserPlaylist["Items"]:
						...
						PlayedItem +=1
						api_url = '{0}Users/{1}/Items/{3}?api_key={2}'.format(SOURCE_URLBASE,user['Id'],SOURCE_APIKEY,item['Id'])
						SOURCE_HEADERS_Auth = {'accept': 'application/json',
						   'api_key': '{0}'.format(SOURCE_APIKEY)}
						response = requests.get(api_url, headers=SOURCE_HEADERS_Auth)   
						if response.status_code == 200:
		...
		print("\n\n\033[92m##### Source Sync Done #####\033[00m\n\n")
